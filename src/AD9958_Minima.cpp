...
float readVoltage(uint8_t val) {return ((val * 5) >> 10);}
...
uint8_t isPinTriggered() {
  if (readVoltage(*ADC140_ADDR01) > TRIGGER_THRESHOLD) {*ADC140_ADCSR |= (0x01 << 15); return A2;}
  if (readVoltage(*ADC140_ADDR09) > TRIGGER_THRESHOLD) {*ADC140_ADCSR |= (0x01 << 15); return A0;}
  isPinOccupied = false;
  return 0;
}

void ADC_SETUP(void)  // Note Set to A1, A0 and A2 commented out
  {
  *MSTP_MSTPCRD &= (0xFFFFFFFF - (0x01 << 16));  // Enable ADC140 module
